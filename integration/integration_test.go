...
func binaryPath(t *testing.T) string {
	...
	if env := os.Getenv("GLASSBOX_BINARY"); env != "" {
		if _, err := os.Stat(env); err == nil {
			return env
		}
		t.Fatalf("GLASSBOX_BINARY is set to %q but the file does not exist", env)
	}
...
func buildBinaryToPath(t *testing.T, repoRoot, binPath string) {
	...
	if err := cmd.Run(); err != nil {
		t.Fatalf("failed to build Glassbox binary: %v\nstderr:\n%s", err, stderr.String())
	}
}
...
func TestVersionFlag(t *testing.T) {
	...
	assertContains(t, "version output", combined, "glassbox")
